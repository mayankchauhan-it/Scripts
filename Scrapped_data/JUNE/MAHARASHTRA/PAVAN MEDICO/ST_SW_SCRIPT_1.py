    ...
    for line in lines:
        ...
        if len(parts) == 11:
            ...
            if parts[-1] == "#":
                ...
                closingQty = parts[-4] #
            elif not parts[-8].isnumeric() or parts[-8] == "-":
                # print("ELIF targeted")
                openingStrip = parts[-7]
                purchaseQty = parts[-6] #Define Purchase Qty Column Number if Formate Changes
                salesQty = parts[-5] #Define Sales Qty Column Number if Formate Changes
        ...
        if len(parts) == 12:
            ...
            if len(parts[-3]) > 3:
                ...
                if not openingStrip.isdigit():
                    # print("IF 1 targeted")
                    openingStrip = parts[-6]
                    purchaseQty = parts[-5]
                    salesQty = parts[-4]
                    closingQty = parts[-2]
                else:
                    # print("else 1 targeted")
                    openingStrip = parts[-7]
                    purchaseQty = parts[-6]
                    salesQty = parts[-5]
                    closingQty = parts[-3] 
                ...
                # print("parts[-4] targeted")
                openingStrip = parts[-7]
                purchaseQty = parts[-6]
                salesQty = parts[-5]
                closingQty = parts[-2]  

            elif len(parts[-5]) > 3 or parts[-4] == "0":
                # print("parts[-5] targeted")
                openingStrip = parts[-8]
                purchaseQty = parts[-7]
                salesQty = parts[-6]
                closingQty = parts[-3]  
